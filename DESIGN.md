...
## 目标与范围
- 重写 `rvisor`（同一二进制内包含控制与服务管理子命令），仅提供 CLI（不做 Web UI 与 XML-RPC）。
- 支持平台：Linux 与 macOS。
- 配置格式采用 TOML，不要求兼容现有 `supervisord.conf`。
- 单进程守护足够：`rvisor` 作为唯一后台进程，运行期控制通过本地 IPC 通信。

## 核心架构
- `rvisor` 负责启动/停止/重启子进程、维护进程状态、采集输出、处理信号。
- `rvisor` 暴露本地 IPC 控制端点（Unix Domain Socket；macOS 同样支持）。
- 同一二进制的 CLI 子命令通过 IPC 与守护进程通信，命令风格尽量贴近现有 `supervisorctl`。
...
- 顶层用法：`rvisor [OPTIONS] <command>`。
- 全局选项：`-c/--configuration`，`-d/--daemon`，`--env-file`。
- 默认前台运行：`rvisor -c /path/to/supervisord.toml`。
- 后台运行：`rvisor -c /path/to/supervisord.toml -d` 或 `--daemon`。
...
2. `cli`：`rvisor run` + `rvisor ctl status/start/stop`。
